...
if vim.g.is_wsl or vim.g.is_win then
    ...
    local function switch_input(lang_code, context)
        return vim.system({ im_select, lang_code }, { text = true }, function()
            -- 任务完成后清理对应的进程引用
            if context == "insert_enter" then
                pending_process.insert_enter = nil
            elseif context == "insert_leave" then
                pending_process.insert_leave = nil
            end
        end)
    end
    vim.schedule(function()
        switch_input("1033", "init")
    end)
    ...
    vim.api.nvim_create_autocmd("InsertLeavePre", {
        ...
        callback = function()
            ...
            vim.defer_fn(function()
                pending_process.insert_leave = switch_input("1033", "insert_leave")
            end, 1)
        end,
    })
    ...
    vim.api.nvim_create_autocmd("InsertEnter", {
        ...
        callback = function()
            ...
            vim.defer_fn(function()
                pending_process.insert_enter = switch_input("2052", "insert_enter")
            end, 1)
        end,
    })
